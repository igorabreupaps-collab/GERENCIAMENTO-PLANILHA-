...
// já calculado e escreve no DOM. Reaproveita quase 1:1 os helpers visuais
// que existiam em build_dashboard.py (RENDERER_JS): esc(), chartCard(),
// statTile(), gaugeRing(), *Row(), previewCard(). A diferença é que agora as
// tabelas editáveis (Áreas / Não Conformidades) chamam de volta callbacks
// (ctx.onX) em vez de acumular estado local -- cada edição salva direto no
// Supabase por meio dessas callbacks, fornecidas pelo app.js.
...
// ---------------------------------------------------------------------
// Agregação: recebe as linhas cruas das tabelas e devolve o objeto D que
// alimenta renderDashboard(). Equivalente ao que extractD()/extract_data.py
// faziam a partir da planilha -- aqui a fonte são as linhas do Supabase.
// ---------------------------------------------------------------------
